...
### 3. 🔍 Yes24.com 도서 & ISBN 정보 2열 스플릿 연동
- **2열 분할 프리뷰**: 좌측 검색 결과 리스트 + 우측 대형 표지, 출판 스펙, **상세 목차 (TOC) 전체 스크롤 열람**, 줄거리 요약
- 도서명, 저자, ISBN-10/13, Yes24 링크 검색으로 표지 이미지, 출판사, 출간일, 목차, 책소개 원클릭 자동 입력

### 4. ✍️ 지식 집필 스튜디오 (Writing Studio & Draft Lab)
- **독서에서 집필로**: 서재에 쌓인 인용구(Quotes)와 AI 토론 통찰을 원클릭으로 참조하여 논문/에세이/칼럼 초고 작성
- **Markdown + LaTeX 듀얼 페인 에디터**: KaTeX 실시간 라이브 렌더링 지원
- **학술 규격 내보내기**: 작성한 초고를 `.md` 및 학술 논문 규격 `.tex` (LaTeX) 파일로 즉시 다운로드

### 5. 🤖 AI 심층 독서 토론 (Google Gemini & OpenAI)
...
### 6. 📐 수학/공학 도서를 위한 LaTeX 수식 지원 & 목차(TOC) 아코디언
- KaTeX 기반 인라인(`$...$`) 및 독립 블록(`$$...$$`) 수식 실시간 렌더링
- 도서 상세 보기 내 **전체 목차 아코디언(TOC)** 상시 열람
- 마우스 드래그로 조절 가능한 **인터랙티브 독서 진행률 슬라이더**

### 7. ☁️ Google Drive 백업 및 복원 (v1.1.0)
- 원클릭 `[GoogleDrive]_MyLibrary_Backup_YYYYMMDD.json` 파일 생성 (도서 및 집필 초고 통합 보관) 및 100% 무손실 복원
